 ...
 * 
 */
public class DeltaStoreBuilderJob extends AbstractHadoopJob {

    private DeltaStoreBuilderConf conf;
    private boolean isAvro;

    public DeltaStoreBuilderJob(String name, Props props) throws Exception {
        super(name, props);
        this.conf = new DeltaStoreBuilderConf(createJobConf(DeltaStoreBuilderMapper.class), props);
        this.isAvro = false;
    }

    public DeltaStoreBuilderJob(String name, Props props, DeltaStoreBuilderConf conf)
    ...
    public DeltaStoreBuilderJob(String name, Props props, boolean isAvro) throws Exception {
        super(name, props);
        this.conf = new DeltaStoreBuilderConf(createJobConf(DeltaStoreBuilderMapper.class), props);
        this.isAvro = isAvro;
    }

    public DeltaStoreBuilderJob(String name, Props props, DeltaStoreBuilderConf conf, boolean isAvro)
                                                                                                     throws FileNotFoundException {
        super(name, props);
        this.conf = conf;
        this.isAvro = isAvro;
    ...
    public static final class DeltaStoreBuilderConf {
        ...
        public DeltaStoreBuilderConf(int replicationFactor,
                                     int chunkSize,
                                     Path tempDir,
                                     Path outputDir,
                                     Path inputPath,
        ...
        public DeltaStoreBuilderConf(JobConf configuration, Props props) throws Exception {
        ...
        // requires job conf in order to get files from the filesystem
        public DeltaStoreBuilderConf(JobConf configuration,
                                     Props props,
                                     String keyField,
                                     String valueField,
                                     String recSchema,
    ...
    public void run() throws Exception {
        ...
        DeltaStoreBuilder builder = null;

        if(isAvro) {

            // NO OP for delta push right now
            /*
             * if(conf.getNumChunks() == -1) { builder = new
             * HadoopStoreBuilder(configuration,
             * 
             * AvroStoreBuilderMapper.class, (Class<? extends InputFormat>)
             * AvroInputFormat.class, cluster, storeDef, chunkSize, tempDir,
             * outputDir, inputPath, checkSumType, saveKeys, reducerPerBucket);
             * } else { builder = new HadoopStoreBuilder(configuration,
             * AvroStoreBuilderMapper.class, (Class<? extends InputFormat>)
             * AvroInputFormat.class, cluster, storeDef, tempDir, outputDir,
             * inputPath, checkSumType, saveKeys, reducerPerBucket,
             * conf.getNumChunks()); }
             * 
             * builder.buildAvro();
             */
        ...
        if(conf.getNumChunks() == -1) {
            builder = new DeltaStoreBuilder(configuration,
                                            DeltaStoreBuilderMapper.class,
