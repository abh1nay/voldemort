...
public class DeltaStoreBuilderMapper extends AbstractStoreBuilderConfigurable implements
    ...
    public Object makeKey(Object key, Object value) {
        return makeResult((BytesWritable) key, keySerializer);
    }

    public Object makeValue(Object key, Object value) {
        return makeResult((BytesWritable) value, valueSerializer);
    }

    private Object makeResult(BytesWritable writable, Serializer serializer) {
    ...
    public void map(BytesWritable key,
        ...
        byte[] keyBytes = key.get();// keySerializer.toBytes(makeKey(key,
                                    // value));
        byte[] valBytes = value.get(); // valueSerializer.toBytes(makeValue(key,
                                       // value));
        ...
        int partitionId = routingStrategy.getMasterPartition(keyBytes);
        byte[] prefixedKey = StoreBinaryFormat.makePrefixedKey(keyBytes, partitionId);

        // sent the k/v pair to the nodes
        for(Node node: nodeList) {

            Integer nodeId = node.getId();
            byte[] nodeArray = intToByteArray(nodeId);
            byte[] nodeprefixedKey = new byte[prefixedKey.length + nodeArray.length];
            System.arraycopy(prefixedKey, 0, nodeprefixedKey, 0, prefixedKey.length);
            System.arraycopy(nodeArray, 0, nodeprefixedKey, prefixedKey.length, nodeArray.length);
            output.collect(new BytesWritable(nodeprefixedKey), new BytesWritable(valBytes));
        }
    ...
    public void configure(JobConf conf) {
        ...
        try {
            ...
            System.out.println(keySerializerDefinition);
            System.out.println(valueSerializerDefinition);
